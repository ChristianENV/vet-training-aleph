...
import { cancelSessionBodySchema } from "@/modules/sessions/validators/sessions";
...
export async function POST(
  ...
  const parsed = cancelSessionBodySchema.safeParse(body);
  if (!parsed.success) {
    return jsonError("Invalid body", 400, { issues: parsed.error.flatten() }, "VALIDATION_ERROR");
  }

  try {
    const session = await cancelSession(gate.user, sessionId);
    if (!session) {
